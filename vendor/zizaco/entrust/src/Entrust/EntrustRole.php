...
class EntrustRole extends Ardent
{
    ...
    {
        return $this->belongsToMany(Config::get('auth.model'), Config::get('entrust::assigned_roles_table'), 'id_users', 'id_roles');
    }
    ...
    public function perms()
    {
        ...
        try {
			return $this->belongsToMany(Config::get('entrust::permission'), Config::get('entrust::permission_role_table'),'id_roles','id_permissions');
        } catch (Exception $e) {
            // do nothing
        }
    }
    ...
    {
        try {
            DB::table(Config::get('entrust::assigned_roles_table'))->where('id_roles', $this->id)->delete();
            DB::table(Config::get('entrust::permission_role_table'))->where('id_roles', $this->id)->delete();
        } catch (Exception $e) {
            // do nothing
        }
    ...
    public function attachPermission($permission)
    {
        ...
        if (is_array($permission)) {
            $permission = $permission['id_permissions'];
        }
