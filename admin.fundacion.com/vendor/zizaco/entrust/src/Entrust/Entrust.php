...
class Entrust
{
    ...
    public function crud()
    {
        ...
        foreach ($models as $model) {
            ...
            $camel    = ucfirst(camel_case($pathinfo['filename']));
            
            $permissions = array_merge($this->permissions);
            
            foreach ($this->permissions as $key => $value)
                $this->routeNeedsPermission($snake.$value,$snake."/".$key);
    ...
    public function routeNeedsPermission($route, $permissions, $result = null, $cumulative=true)
    {
        ...
        if (!$result instanceof Closure) {
            ...
            $result = function ($_route, $_request) use ($route, $permissions, $result, $cumulative) {

                $exist_segment = ends_with($route,"*");
                $exist_segment = $exist_segment ? $_request->segment(3) : $exist_segment;
