...
class Router implements HttpKernelInterface, RouteFiltererInterface {
	...
	public function crud($controller = 'admin\CrudController',$prefix = '')
	{
        ...
        if(!empty($prefix) and !str_contains($prefix,"/"))
        {
        	$prefix= $prefix."/";
        }	
	...
	/**
	 * Add the print method for a resourceful route.
	 *
	 * @param  string  $name
	 * @param  string  $base
	 * @param  string  $controller
	 ...
	 * @return \Illuminate\Routing\Route
	 */
	protected function addResourcePrint($name, $base, $controller, $options)
	{
	 ...
	 * @return \Illuminate\Routing\Route
	 */
	protected function addResourceTab($name, $base, $controller, $options)
	{
		$uri = $this->getResourceUri($name).'/{'.$base.'}/tab/{tab}';
		
		$action = $this->getResourceAction($name, $controller, 'tab', $options);
